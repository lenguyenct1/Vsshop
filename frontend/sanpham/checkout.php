...
if (!isset($_SESSION['kh_tendangnhap_logged_frontend']) || empty($_SESSION['kh_tendangnhap_logged_frontend'])) {
	echo 'Vui lòng Đăng nhập trước khi Thanh toán! <a href="/Vsshop/frontend/quanly/login.php">Click vào đây để đến trang Đăng nhập</a>';
	die;
} else {
	...
	--- 
	*/
	$kh_tendangnhap = $_SESSION['kh_tendangnhap_logged_frontend'];
...
EOT;
	...
	INSERT INTO `dondathang` (`dh_ngaylap`, `dh_noigiao`, `dh_trangthaithanhtoan`, `kh_tendangnhap`) 
		VALUES ('$dh_ngaylap', N'$dh_noigiao', '$dh_trangthaithanhtoan', '$kh_tendangnhap');
EOT;
...
EOT;
		...
		INSERT INTO `sanpham_dondathang` (`sp_id`, `dh_id`, `sp_dh_soluong`, `sp_dh_dongia`)
			VALUES ($sp_id, $dh_id, $sp_dh_soluong, $sp_dh_dongia);
EOT;
	...
	//	$mail->SMTPDebug = 2;                                   // Enable verbose debug output
		...
		// Tiêu đề Mail
		$mail->Subject = "[Có đơn hàng vừa thanh toán] - Mã đơn hàng $dh_id";
		...
		$body = <<<EOT
			<table border="1" width="100%">
				<tr>
					<td colspan="2">
						<img src="http://localhost/Vsshop/assets/vendor/frontend/img/logo.png" style="width: 100px; height: 100px; border: 1px solid red;" />
					</td>
				</tr>
				<tr>
...
EOT;
	...
	echo 'Đặt hàng thành công. <a href="/Vsshop">Bấm vào đây để quay về trang chủ</a>';
}
